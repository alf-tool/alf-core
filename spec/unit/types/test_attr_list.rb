  ...
  describe AttrList do
    ...
    describe "split" do 
      ...
      specify "the documentation example" do
        ...
        list.split(tuple).should eq([{:name => "Jones"}, {:city => "London"}])
        list.split(tuple, true).should eq([{:city => "London"}, {:name => "Jones"}])
      end
    ...
    describe "project" do 
      ...
      describe "when used with allbut set to true" do
        subject{ key.project(tuple, true) }
        it{ should eq({:c => 3}) }
      end

      describe "when used without allbut set to false" do
        subject{ key.project(tuple, false) }
        it{ should eq({:a => 1, :b => 2}) }
      end
      ...
      specify "the documentation example" do
        list = AttrList.new([:name])
        tuple = {:name => "Jones", :city => "London"}
        list.project(tuple).should eq({:name => "Jones"})
        list.project(tuple, true).should eq({:city => "London"})
      end
